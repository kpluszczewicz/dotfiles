...
local path = "/Users/kamilpluszczewicz/koleo/db/structure.sql"
local i = 1
local matching_lines = 0
for line in io.lines(path) do
	local _, _, table_name = string.find(line, "^CREATE TABLE public[.](.*) [()]")

	if table_name then
		table.insert(lines_with_numbers, { lnum = i, table_name = table_name, path = path })
		matching_lines = matching_lines + 1
	end
	i = i + 1
end

local koleo_db_tables_picker = function(opts)
	opts = opts or {}
	pickers
		.new(opts, {
			prompt_title = "Koleo database tables",
			finder = finders.new_table({
				results = lines_with_numbers,
				entry_maker = function(entry)
					return {
						value = entry,
						display = entry.table_name,
						ordinal = entry.table_name,
						lnum = entry.lnum,
						path = entry.path,
					}
				end,
...
vim.keymap.set("n", "<leader>kt", koleo_db_tables_picker, { desc = "[K]oleo db tables search" })
